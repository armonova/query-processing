...
class ProcessingTest(unittest.TestCase):
    ...
    def test_count_top_n_relevant(self):
        ...
        for n, arr_resp_esperada in resp_esperada_por_top_n.items():
            ...
            for i, resp_esperada in enumerate(arr_resp_esperada):
                ...
                # print()
                self.assertEqual(resp_esperada, resposta,
                                 msg=f"# de relevantes esperadas top {n}: {resp_esperada} resposta obtida: {resposta}")
            ...
            for occur_resp in lst_occur_resp:
                ...
                for expected_occur in map_expected_resp_list[term]:
                    if expected_occur.term_id == occur_resp.term_id and expected_occur.doc_id == occur_resp.doc_id:
                        ...
                        bol_encontrou = True
                self.assertTrue(bol_encontrou,
                                msg=f"Não foi possível encontrar o termo '{term}' do documento {expected_occur.doc_id}")
    ...
    def test_get_relevance_per_query(self):
        ...
        self.assertListEqual(list(dic_relevance_docs.keys()), expected_relevance_docs,
                             f"A resposta '{dic_relevance_docs}' deveria ser {expected_relevance_docs}")
